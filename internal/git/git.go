...
// front: git would refuse the new rebase, and the leftover state belongs to
// the earlier one, so its conflicted files must not be mistaken for this
// rebase's conflicts.
...
// errRebaseAlreadyInProgress is returned when a rebase is started while
// another one is unresolved.
var errRebaseAlreadyInProgress = errors.New("a rebase is already in progress — resolve or abort it first")

// RebaseStartError indicates that a git rebase command failed before it
// created any rebase state, meaning nothing was rebased at all. Common
// causes are a dirty working tree, a branch that is checked out in another
// worktree, an upstream ref that does not resolve, or a rebase that is
// already in progress.
//
// It is distinct from a rebase that started and stopped on a conflict:
// there is nothing to `--continue` or `--abort`, and callers must treat it
// as a hard failure rather than a recoverable conflict.
...
// IsRebaseStartError reports whether err indicates a rebase that never
// started (see RebaseStartError).
...
// started with (e.g. cdate_is_adate for --committer-date-is-author-date) in
// the rebase state directory, so they are honored automatically.
...
//
// If no rebase is in progress on the very first check, the rebase never
// started and originalErr is returned wrapped in a *RebaseStartError.
// Treating that case as success is what previously made a rebase that did
// nothing report "rebased successfully". On later iterations the same
// condition legitimately means the auto-continued rebase finished.
